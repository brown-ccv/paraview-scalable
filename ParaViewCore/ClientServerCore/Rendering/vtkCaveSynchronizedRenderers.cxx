    ...
    for (int cc = 0; cc < this->NumberOfDisplays; cc++)
    {
      if (options->GetDisplayName(cc))
      {
        if (this->ParallelController && this->ParallelController->GetLocalProcessId() == cc)
        {
          ...
          #ifdef USE_SCALABLE 
		this->ScalablePolFile = options->GetScalablePol(cc);
                std::string strFile = this->ScalablePolFile;
                std::string strMachine = options->GetMachineName(cc)  ;
                std::cout<< "SETTING SCALABLE POL FILE = " + strFile + " " +strMachine << std::endl;
          #endif
        }
      }
      this->DefineDisplay(
...
void vtkCaveSynchronizedRenderers::ComputeCamera(vtkCamera* camera)
{
  if (once)
  {
   ...
   #ifdef USE_SCALABLE
    this->GetRenderer()->ActivateScalable();
    camera->SetScalablePolFile(this->ScalablePolFile);
   #endif
