...
vtkOpenGLRenderer::vtkOpenGLRenderer()
{
  ...
  this->debugFile.open("myDebug.txt");


#ifdef USE_SCALABLE
  this->ScalableActivated = true;
  this->ScalableInitialized = false;
  this->gmSDKL = NULL;
  this->gmSDKR = NULL;
#endif
...
void vtkOpenGLRenderer::DeviceRender(void)
{
  ...
  else
  {
    ...
    //Set up scalable
#ifdef USE_SCALABLE
	if(this->IsScalableActivated())
	{
           

	  if(!(this->IsScalableInitialized()))
	  {

           std::cout << "Initializing Scalable" << std::endl;
           //this->debugFile << "Scalable is about to initialize \n";
	   std::cout  << "Scalable initialization starts \n";
            //Set up the Scalable plugin
	    this->InitializeScalable();
	    this->gmSDKL = new MeshSDK_Mesh;
	    this->gmSDKR = new MeshSDK_Mesh;
	    
	    if(this->ActiveCamera->GetScalablePolFile() != NULL)
	    {
                        std::cout  << " before glGetString " << std::endl;
			std::cout << glGetString(GL_VERSION) << std::endl;
                        std::cout  << " after glGetString " << std::endl;
                        EasyBlendSDKError msdkErrL = EasyBlendSDK_Initialize(this->ActiveCamera->GetScalablePolFile(), gmSDKL);
			EasyBlendSDKError msdkErrR = EasyBlendSDK_Initialize(this->ActiveCamera->GetScalablePolFile(), gmSDKR);
                        std::cout  << " after EasyBlendSDK_Initialize &  EasyBlendSDK_Initialize" << std::endl;
			
			if ( msdkErrL != EasyBlendSDK_ERR_S_OK &&  msdkErrR != EasyBlendSDK_ERR_S_OK)
			{
				std::cout << "Error on Init: " << EasyBlendSDK_GetErrorMessage(msdkErrL) << EasyBlendSDK_GetErrorMessage(msdkErrR) << std::endl;
			}
			else
			{
				vtkOpenGLRenderWindow *win = vtkOpenGLRenderWindow::SafeDownCast(this->GetRenderWindow());
			
				win->SetScalableLeftBackBufferMesh(gmSDKL);
			    win->SetScalableRightBackBufferMesh(gmSDKR);
			    
				EasyBlendSDK_SetInputReadBuffer(gmSDKL,  GL_BACK_LEFT);
				EasyBlendSDK_SetOutputDrawBuffer(gmSDKL,  GL_BACK_LEFT);
				
				EasyBlendSDK_SetInputReadBuffer(gmSDKR,  GL_BACK_RIGHT);
				EasyBlendSDK_SetOutputDrawBuffer(gmSDKR,  GL_BACK_RIGHT);
				
				this->ActiveCamera->SetGMSDKLeft(gmSDKL);
				this->ActiveCamera->SetGMSDKRight(gmSDKR);
                                //this->InitializeScalable();
                               // this->debugFile << "Scalable initialization done \n";
                               std::cout  << "Scalable initialization done \n";
				
		      }
	  }
	  else
	  {
                        //this->debugFile << "Scalable initialization failed \n";
	      std::cout<<"Scalable not configured for this screen" << std::endl;
	      gmSDKL = NULL;
	      gmSDKR = NULL;
	  }  
       }
       ...
       {
            std::cout  << "Scalable already initialized \n";
       }
    }
#endif
