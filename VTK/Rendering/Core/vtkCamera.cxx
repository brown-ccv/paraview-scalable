...
#ifdef USE_SCALABLE
  this->ScalablePolFile = NULL;
  //Do we have to shutdown the gmSDK?
  if(this->gmSDKL != NULL)
	delete this->gmSDKL;
  if(this->gmSDKR != NULL)
	delete this->gmSDKR;
...
void computeTileCornerPoint(EasyBlendSDK_Mesh *mesh, double xang,  double yang, double* eye  , double *corner)
{
  const double deg2rad = M_PI/180.0;
    
	corner[0] = tan(deg2rad * xang);
	corner[1] = tan(deg2rad * yang);
	corner[2] = -1.0f;
  
	rotateVec(-deg2rad*(mesh->Frustum.ViewAngleA), 0, 0, 1, corner[0], corner[1], corner[2]);
	rotateVec(-deg2rad*(mesh->Frustum.ViewAngleB), 0, 1, 0, corner[0], corner[1], corner[2]);
	rotateVec(-deg2rad*(mesh->Frustum.ViewAngleC - 90), 1, 0, 0, corner[0], corner[1], corner[2]);
	
	corner[0] =  corner[0] + eye[0];
	corner[1] =  corner[1] + eye[1];
	corner[2] =  corner[2] + eye[2];

	corner[0] = corner[0] / 3.28084;
	corner[1] = corner[1] / 3.28084;
	corner[2] = corner[2] / 3.28084;
...
void vtkCamera::UpdateScalableCorners()
{
  ...
  { 
	 ...
	 EasyBlendSDK_SetEyepoint(mesh , E[0], E[1], E[2]);
	  
	 //BottomLeft
	 computeTileCornerPoint(mesh, mesh->Frustum.LeftAngle, mesh->Frustum.BottomAngle, &E[0], &this->ScreenBottomLeft[0]);
	 //TopRight
	 computeTileCornerPoint(mesh, mesh->Frustum.RightAngle, mesh->Frustum.TopAngle, &E[0], &this->ScreenTopRight[0]);
	 //BottomRight
	 computeTileCornerPoint(mesh, mesh->Frustum.RightAngle, mesh->Frustum.BottomAngle, &E[0], &this->ScreenBottomRight[0]);                                           
  }  
}
#endif
...
#ifdef USE_SCALABLE
void vtkCamera::SetScalablePolFile(const char* file)
{
	this->ScalablePolFile = file;
}

const char* vtkCamera::GetScalablePolFile()
{
	return this->ScalablePolFile;
}

void vtkCamera::SetGMSDKLeft(EasyBlendSDK_Mesh *gmSDK)
{
	this->gmSDKL = gmSDK;
}

void vtkCamera::SetGMSDKRight(EasyBlendSDK_Mesh *gmSDK)
{
	this->gmSDKR = gmSDK;
}
#endif
