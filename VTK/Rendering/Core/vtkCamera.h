...
#ifdef USE_SCALABLE
  void SetScalablePolFile(const char* path);
  const char* GetScalablePolFile();
  void SetGMSDKLeft(EasyBlendSDK_Mesh *gmSDK);
  void SetGMSDKRight(EasyBlendSDK_Mesh *gmSDK);
  void UpdateScalableCorners();
#endif
...
#ifdef USE_SCALABLE
  const char* ScalablePolFile;
  EasyBlendSDK_Mesh *gmSDKL;
  EasyBlendSDK_Mesh *gmSDKR;
#endif
